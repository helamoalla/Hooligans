 ...
 * @author Nadia
 */
public class ShowTicket extends BaseForm {
      public ShowTicket(Resources res) throws ParseException{    
...
//        
        ...
        TicketService ps = TicketService.getInstance();
     
     
     List<Ticket> list = ps.fetchTicket();
       for (Ticket p : list) {
    Container produitContainer = new Container(new BorderLayout());
    Container infosContainer = new Container(BoxLayout.y());
           
     // Récupération de l'image à partir du code QR
   ...
   Label nomEventLabel = new Label("Num ticket " + p.getNum_ticket());
    Label nomlabel = new Label("Nom user: " + SessionManager.getNom());
//    Label nomevent = new Label("nom event " +p.getEvent().getNom_event()  );
      Label maillabel = new Label("mail: " + SessionManager.getEmail());

   
   TicketService TS = new TicketService();
   infosContainer.add(nomEventLabel);
    infosContainer.add(nomlabel);
   // infosContainer.add(nomevent);
  infosContainer.add(maillabel);
  Button imprimer = new Button("Imprimer ");
       imprimer.addActionListener(e -> { 
           TS.imprimer(p.getId_ticket());
       }
        
   );
    
    Container buttonsContainer = new Container(BoxLayout.x());
  buttonsContainer.addAll(imprimer);
