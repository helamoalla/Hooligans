...
public class ShowEventForm extends BaseForm {
      public ShowEventForm(Resources res) {    
     ...
     List<Event> list = ps.fetchEvent();
          for (Event l : list) {
        addButton(l.getImage(),l.getNom_event(), l.getPrix(), l.getDate_debut().toString(),l.getDate_fin().toString(), l.getLieu_event(),l.getType_event());
          }
...
//}
   ...
   private void addButton(String img, String title, double prix, String datedebut,String datefin, String lieu,String type) {
      ...
      ImageViewer image1 = new ImageViewer(urlImage.scaledWidth(Math.round(Display.getInstance().getDisplayWidth() * 0.2f)));
       ...
       TextArea ta = new TextArea("Nom du Event "+title);
   ...
   Button AffichedetailButton = new Button("Afficher Detail");
       ...
       cnt.add(BorderLayout.CENTER, 
               BoxLayout.encloseY(
                       ta,p,qt,sm,
                       BoxLayout.encloseX(deleteButton)
               ));
       add(cnt);
